...
class Menu(DatabaseManager):
    def __init__(self):
        super().__init__()
        cur.execute("CREATE TABLE IF NOT EXISTS MENU(id INT, title TEXT, description TEXT, price REAL, img_url TEXT)")
        self.deletetable_execute = "DROP TABLE MENU"
        self.data = ["id","title", "description", "price", "Img url"]
    ...
    def importjson(self,jsondict):
        ...
        for item in jsondict['item']:
            data.append([item["id"],item["title"],item["cont"],item["price"],item["img_url"]]) #case sensative
        for d in data:
            print (d)
